...
Always reply in the user's language: Spanish if they write in Spanish, English if they write in English. Work items created in Azure DevOps follow the language defined in `references/organization.md`.

## Organization Conventions (MANDATORY)

Before reading from or writing to Azure DevOps, read `references/organization.md`.
Organization rules take precedence over any generic practice in this skill.

- Using the `azure-devops` MCP server (tools, confirmations, WIQL) → `references/azure-devops.md`.
- Creating or editing work items, test plans or test cases → use `references/templates.md`.
- Deciding whether an item is ready (DoR) or done (DoD) → use `references/definitions.md`.
  These definitions replace the generic Definition of Done below.
- If a value in those files says `<FILL IN>`, do not invent it: ask the user.
- If a rule cannot be followed (missing field, missing permission, etc.), say so and ask; never skip it silently.
- If the user asks for something that contradicts a rule, point it out before acting.
...
This organization uses Azure DevOps through the `azure-devops` MCP server. See `references/azure-devops.md` for tools, WIQL queries and write rules, and `references/integrations.md` for setup and troubleshooting.
